...
def locate_dash(text, form, sec):
    ...
    ENDS = {("10-K","MDA"): [("7A", r"quantitative"), ("8", r"financial\s+statements")],
            ("10-K","RISK_FACTORS"): [("1B", r"unresolved"), ("2", r"properties")],
            ("10-Q","MDA"): [("3", r"quantitative"), ("4", r"controls\s+and\s+procedures")],
            ("10-Q","RISK_FACTORS"): [("2", r"unregistered"), ("3", r"defaults"), ("4", r"mine\s+safety"),
                                       ("5", r"other\s+information"), ("6", r"exhibits")]}
    end_pats = [dash_pattern(a, b) for a, b in ENDS[(form, sec)]]
...
pd.DataFrame(out).to_csv("data/f3/p4_redteam/d9_check_bothends.csv", index=False)
