...
**Status: PLAN. Nothing in F5 has started. Gate G2 was ruled "proceed under the
...
m-chunk composition with m stated — never a G2 chunk proportion). §12 carries
...
|---|---|---|---|
| 1 | Benchmark (EXPANSION_PLAN §3.3) | equal-weighted mean over that date's members **excluding self**, membership-dated (already implemented in `controls.py`); E1/E2 numbers declared incomparable wherever both appear | OWNER-GATE G3 |
...
| owner time | one ~3-page G3 read + the 19 rulings above; one per-fold read at Step 4 |
